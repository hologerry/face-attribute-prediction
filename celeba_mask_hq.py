...
class CelebAMaskHQSingle(data.Dataset):
    def __init__(self, data_root, dataset_name, mode, transform_img, transform_seg=None, selected_attrs=None, seg_channel=0, single_attr=None):
        super().__init__()
        self.data_root = data_root
        self.dataset_name = dataset_name
        self.mode = mode
        ...
        self.preprocess()
        self.num_image_attr_pairs = len(self.train_dataset) if self.mode == 'train' else len(self.test_dataset)
        self.select_attr_idx = self.attr2idx[single_attr]
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            filename_a, label_a = self.train_dataset[index]
            label_a = [label_a[self.select_attr_idx]]
            image_a = Image.open(ospj(self.image_dir, filename_a))
            return self.transform_img(image_a), torch.LongTensor(label_a)
